...
clase = SpongeBobFinder.Class;
numImages = height(SpongeBobFinder); % or length(taula) if it's a struct array
numBins = 64; % Number of bins for the histogram
tamImage = 256; 

% Initialize arrays to store histograms
HUESatcosHist = zeros(numImages, numBins);
HUESatsinHist = zeros(numImages, numBins);
...
% Read the image
img = imread(imgPath);
imgres = imresize(img, tamresize, 'bilinear');
figure, imshow(imgres), title("imatge");
[fvector, Visualize] = extractHOGFeatures(imgres, 'CellSize',[numBins/2, numBins/2],'NumBins',10000);
hold on
plot(Visualize);
HOG = calcularAngulosHOG(imgres, [numBins/2, numBins/2],100000);

%%

myFold = dir(".\TRAIN\Bob esponja\*.jpg");
imgPath = fullfile(myFold(3).folder, myFold(3).name);

img = imread(imgPath);
figure, imshow(rgb2hsv(img)), title("HSV image");
hold on
colormap("hsv")


valuesHSV = rgb2hsv(img);

limiteHSV = valuesHSV(1:100,1:100,:);
...
for i = 1:numImages
    ...
    % Construct full image path
        imgPath = fullfile(myFold(i).folder, myFold(i).name);
        ...
        hsvImg = rgb2hsv(imgres);
        
        % Extract the H, S, and V channels
        H = hsvImg(:,:,1); % Hue channel
        S = hsvImg(:,:,2); % Saturation channel
        V = hsvImg(:,:,3); % Value channel
        
        % Normalize channels (if required)
        % For example, you could normalize hue and saturation if necessary
        % Normalization is optional, depending on your desired feature set
        
        % Calculate histograms for Hue and Saturation
        HUESatcosHist(i, :) = imhist(cos(2*pi*H).*S, numBins)';
        HUESatsinHist(i, :) = imhist(sin(2*pi*H).*S, numBins)';
        
        % Compute HOG Features (using grayscale image for HOG)
        grayImg = rgb2gray(imgres); % HOG requires grayscale
        hogFeatures(i, :) = extractHOGFeatures(grayImg, 'CellSize', [cell_size,cell_size]);
...
%%
save('MatBobEsponjaHSV.mat', 'HUESatcosHist', 'HUESatsinHist', "hogFeatures", 'clase');

%%
load('MatBobEsponjaHSV.mat');

%%
Feature = table(clase, HUESatsinHist, HUESatcosHist, hogFeatures, ...
    'VariableNames', {'Class', 'HUESatsinHist','HUESatcosHist', 'HogHist'});